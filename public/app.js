...
async function summarize(timeRange, project, include_knowledge) {
  return api('/api/summarize', {
    method: 'POST',
    body: JSON.stringify({ timeRange, project, include_knowledge }),
  });
}
...
function renderKnowledgeCard(k) {
  const canDelete = state.auth?.is_admin || k.author_name === state.auth?.name;
  return `
    <div class="knowledge-card" data-id="${escapeHtml(k.id)}">
      ...
      <div class="knowledge-card-body" hidden>
        ...
        <div class="knowledge-card-foot">
          <span>${escapeHtml(k.author_emoji || '🤖')} ${escapeHtml(k.author_name || '')} · ${formatCardDateTime(k.created_at)}</span>
          ${canDelete ? '<button class="knowledge-delete" aria-label="删除知识卡">✕</button>' : ''}
        </div>
      </div>
    </div>
  `;
  ...
  const groupsHtml = groups.map(g => `
    <div class="date-group">
      ...
      ${g.notes.map(n => {
        ...
        const canMutate = state.auth?.is_admin || n.author_name === state.auth?.name;
        const isSummary = n.is_summary == 1 || n.card_type === 'summary';
...
async function runSummarize() {
  ...
  try {
    const data = await summarize(timeRange, project, include_knowledge);
    lastSummary = { ...data, timeRange, project };
    $('sum-loading').hidden = true;
    $('sum-result').hidden = false;
    const meta = data.meta || {};
    const knowledgeTag = meta.include_knowledge ? ' · 含知识卡' : '';
    $('sum-meta').textContent = `近 ${meta.days} 天 · ${meta.project === 'all' ? '全部项目' : meta.project} · ${meta.noteCount} 条${knowledgeTag}`;
